...
class MultiLayerPerceptron4Predictor(nn.Module):
    def __init__(self, input_size: int, output_size: int):
        super(MultiLayerPerceptron4Predictor, self).__init__()
    ...
    def get_default_save_location(self, use_inference_model: bool = False) -> Path:
        ...
        return Path(save_dir, f'{file_name[0]}4_{uuid}__EPOCHS={self.current_epoch}_LOSS={self.loss_history[-1]:.3}.{file_name[1]}')
    ...
    def load(optimizer: torch.optim.Optimizer, path: os.PathLike = None) -> tuple[nn.Module, torch.optim.Optimizer]:
        if path is None:
            path = MultiLayerPerceptron4Predictor.get_latest_model_from_default_location()
            logging.info(f"Using default save location: {path}")
        checkpoint = torch.load(path)

        input_size = checkpoint['input_size']
        loaded_model = MultiLayerPerceptron4Predictor(input_size, 1)

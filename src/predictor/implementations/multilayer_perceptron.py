...
class MultiLayerPerceptronPredictor(nn.Module):
    ...
    def get_default_save_location(self, use_inference_model: bool = False) -> Path:
        ...
        return Path(save_dir, f'{file_name[0]}{self.version}_{uuid}__EPOCHS={self.current_epoch}_{self.optimizer}_LR={self.lr}_LOSS={self.loss_history[-1]:.3}.{file_name[1]}')

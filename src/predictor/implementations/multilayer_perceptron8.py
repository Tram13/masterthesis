...
class MultiLayerPerceptron8Predictor(nn.Module):
    def __init__(self, input_size: int, output_size: int):
        super(MultiLayerPerceptron8Predictor, self).__init__()
        ...
        self.linear_stack = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(p=0.2),
            nn.Linear(int(input_size * 1.2), int(input_size * 1.5)),
            nn.ReLU(),
            nn.Dropout(p=0.2),
            nn.Linear(int(input_size * 1.5), int(input_size * 1.2)),
            nn.ReLU(),
            nn.Dropout(p=0.2),
        ...
        torch.nn.init.xavier_uniform_(self.linear_stack[17].weight)
        torch.nn.init.xavier_uniform_(self.linear_stack[19].weight)
    ...
    def get_default_save_location(self, use_inference_model: bool = False) -> Path:
        ...
        return Path(save_dir, f'{file_name[0]}8_{uuid}__EPOCHS={self.current_epoch}_LOSS={self.loss_history[-1]:.3}.{file_name[1]}')
    ...
    def load(optimizer: torch.optim.Optimizer, path: os.PathLike = None) -> tuple[nn.Module, torch.optim.Optimizer]:
        if path is None:
            path = MultiLayerPerceptron8Predictor.get_latest_model_from_default_location()
            logging.info(f"Using default save location: {path}")
        checkpoint = torch.load(path)

        input_size = checkpoint['input_size']
        loaded_model = MultiLayerPerceptron8Predictor(input_size, 1)
